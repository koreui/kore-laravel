...
return [
    ...
    /*
    |--------------------------------------------------------------------------
    | spatie/laravel-medialibrary — config publicada
    |--------------------------------------------------------------------------
    |
    | Publicado con `php artisan vendor:publish --tag=medialibrary-config` y
    | tocado en tres sitios; el resto es el archivo del paquete tal cual, para
    | que un `diff -w` contra `vendor/spatie/laravel-medialibrary/config/` al
    | actualizar enseñe sólo lo que cambia el paquete. (Con una cuarta
    | diferencia que no es una decisión: Pint le añade el
    | `declare(strict_types=1)` y Rector le quita el `, null` redundante de
    | `env('MEDIA_CONVERSIONS_DISK', null)`. Pasa con todos los configs
    | publicados; ver la nota de `config/kore-ui.php` en CLAUDE.md.)
    |
    |   1. `disk_name` lee `FILES_DISK` (y su default es `local`, no `public`):
    |      el disco de los archivos lo decide el módulo Files, y por defecto es
    |      privado. R12 manda leerlo de `env()` y no de `config('files.disk')`,
    |      porque los `config/*.php` se cargan en orden alfabético y `files`
    |      todavía no existe cuando esto se evalúa. La cifra vive en dos sitios
    |      y lo verifica `FilesConfigTest`.
    |   2. `path_generator` es el del módulo, que reparte los ficheros por
    |      dueño y colección en vez de por id de media suelto.
    |   3. `max_file_size` sube a 50 MB para acompañar a `files.max_upload_kb`
    |      (51200 KB). Si el paquete cortara antes que la validación, un archivo
    |      admitido por el formulario reventaría al guardarse.
    |
    ...
    */
     ...
     */
    'disk_name' => env('FILES_DISK', 'local'),
     ...
     */
    'max_file_size' => 1024 * 1024 * 50, // 50MB — acompaña a files.max_upload_kb
     ...
     */
    'path_generator' => SlotPathGenerator::class,
